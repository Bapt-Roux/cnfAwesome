...
function screen:init(args)
  ...
  awful.button({}, 1, function () env.kdbLayout('dvp') end),
  awful.button({}, 3, function () env.kdbLayout('fr') end)
  )
  ...
  function(s) -- {{{ Master
    if s.index == 1 then
      -- wallpaper
      env.wallpaper(s, "wallpaper/normal")

      -- tags
      awful.tag({ "w1", "w2", "w3", "w4", "w5" }, s, 
    ...
    env.wrapper(tray.widget, "tray", tray.buttons),
        ...
        -- wallpaper
        env.wallpaper(s, "wallpaper/rotate")

        -- tags
        awful.tag({ "Web", "Pdf", "Various", "Task", "Mail" }, s, 
